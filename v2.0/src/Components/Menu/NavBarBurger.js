...
class NavBarBurger extends React.Component {
  ...
  renderLogin() {
    ...
    if (user.info) {
      ...
      console.log(user);
      const profileBtnColor = user.info.user_info && user.info.user_info.prefs && user.info.user_info.prefs.color;
      ...
      return (
        <Dropdown onSelect={() => null} className="d-flex h-auto">
          <Dropdown.Toggle style={{"background-color":"white", "border-color":"white"}}>
            {user.info.profile_picture && user.info.profile_picture.url ? 
              <img src={user.info.profile_picture.url}
                alt={user.info.full_name + " profile"}
                style={{
                  "height":50,
                  "border-radius":"50%"
                }}></img> : 
              <img src={createImagefromInitials(profileBtnColor, user.info.full_name, 50)}
                alt={user.info.full_name + " profile"}
                style={{
                  "border-radius":"50%"
                }}></img> }
